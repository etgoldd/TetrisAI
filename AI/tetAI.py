...
class TetAI:
    ...
    def __init__(self, board, parents: [None, list[TetAI]], max_mutation: float, mutation_chance: int):
        ...
        self.layer1 = Layer(n_inputs=N_INPUTS, n_neurons=LAYER1_NEURONS, activation_function="ReLU")
        self.output_layer = Layer(n_inputs=LAYER1_NEURONS, n_neurons=1, activation_function="None")
        ...
        if parents is not None:
            ...
            if len(parents) == 2:
                ...
                self.layer1.weights = self.combine_wirings([parent0.layer1.weights, parent1.layer1.weights])
                self.layer1.biases = self.combine_wirings([parent0.layer1.biases, parent1.layer1.biases])

                self.output_layer.weights = self.combine_wirings([parent0.output_layer.weights, parent1.output_layer.weights])
                self.output_layer.biases = self.combine_wirings([parent0.output_layer.biases, parent1.output_layer.biases])
            else:
                self.layer1.weights = self.parents[0].layer1.weights + self.randomize_mutation(self.layer1.weights.shape)
                self.layer1.biases = self.parents[0].layer1.biases + self.randomize_mutation(self.layer1.biases.shape)

                self.output_layer.weights = self.parents[0].output_layer.weights + self.randomize_mutation(self.output_layer.weights.shape)
                self.output_layer.biases = self.parents[0].output_layer.biases + self.randomize_mutation(self.output_layer.biases.shape)
    ...
    def randomize_mutation(self, shape: [tuple, list, np.ndarray]):
            ...
            for row in range(len(mutation)):
                for column in range(len(mutation[0])):
                    ...
                    if randomizer > adjusted_stability:
                        mutation[row][column] = 2 * self.max_mutation * np.random.random() - self.max_mutation
        else:
            for index in range(shape_d):
                randomizer = np.random.randint(1, 200)

                if randomizer > adjusted_stability:
                    mutation[index] = 2 * self.max_mutation * np.random.random() - self.max_mutation
        ...
        if len(shape) == 2:
            for row in range(shape[0]):
                for column in range(shape[1]):
                    new_wiring[row][column] = parents_layer_matrices[randomizer[row][column]][row][column]
        else:
...
class Layer:
    ...
    def __init__(self, n_inputs: int,  n_neurons: int, activation_function: str):
        self.weights: np.ndarray = np.maximum(np.minimum(0.3 * np.random.randn(n_inputs, n_neurons), 1), -1)
    ...
    def forward(self, inputs: np.ndarray):
        ...
        elif self.activation_function_type == 'None':  # Not needed at all, but here just to make things more orderly
            pass

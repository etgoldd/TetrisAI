...
simulation = _simulation.Simulation(gen_size=22,
...
RED_TUPLE = (255, 60, 60)
#
# def blit_network(brain: _tetAI.TetAI):
#
#     # Blitting the neurons
#     for i in range(_tetAI.N_INPUTS):
#         # WIN.blit(neuron_image, (RIGHT_AREA_OFFSET + (NON_GRID_WIDTH_RIGHT // 3 - neuron_image.get_width()) / 2, NETWORK_LOCATION + i * 50 + 100))
#         WIN.blit(neuron_image, (RIGHT_AREA_OFFSET + (NON_GRID_WIDTH_RIGHT // 3 - neuron_image.get_width()) / 2, NETWORK_LOCATION + i * 50 + 100))
#     for i in range(_tetAI.LAYER1_NEURONS):
#         WIN.blit(neuron_image, (RIGHT_AREA_OFFSET + (NON_GRID_WIDTH_RIGHT - neuron_image.get_width()) / 2, NETWORK_LOCATION + i * 50))
#     WIN.blit(neuron_image, (RIGHT_AREA_OFFSET + (NON_GRID_WIDTH_RIGHT + 316 - neuron_image.get_width()) / 2, NETWORK_LOCATION + 225))
#
#     # Blitting the connections
#     layer1_weights_surfaces = []
#     for neuron_index, neuron_connections in enumerate(brain.layer1.weights):
#         neuron_weights_surfaces = []
#         for i, weight in enumerate(neuron_connections):
#             neuron_weights_surfaces.append(pyg.)
#         layer1_weights_surfaces.append(neuron_weights_surfaces)


def draw_window(grid_lines_vert: list[pyg.Rect], grid_lines_horiz: list[pyg.Rect], board: tet.TetrisBoard, simulation: _simulation.Simulation):
    ...
    generation_image = text_font.render(f"{simulation.generation + 1}", True, (255, 255, 255))
    ...
    unit_image = text_font.render(f"{simulation.tetting_index + 1}", True, (255, 255, 255))
...
def main():
    ...
    tetris_board = simulation.active_tetting.board
    simulation.next_piece()
    t = 1
    ...
    while run:
        ...
        if simulation.dead_tetting_flag:
            tetris_board = simulation.active_tetting.board
